...
export default defineComponent({
  ...
  setup (props, context) {
    ...
    /**
     * 采用递归的方式遍历树，以便于在遇到未展开的节点时及时返回
     * @author zhiliang.yang
     */
    const handleVirtualTreeWithBreak = (row: any, fullData: any[], expandMaps: any, treeExpandedMaps: any, fullAllDataRowIdData: any, treeIndex:number) => {
      const treeOpts = computeTreeOpts.value
      ...
      if (!parentRowRest || (expandMaps[parentRowid] && treeExpandedMaps[parentRowid])) {
        const rowRest = fullAllDataRowIdData[rowid]
        if (rowRest) {
          rowRest._index = fullData.length
          rowRest.treeIndex = treeIndex
          rowRest.seq = parentRowRest ? `${parentRowRest.seq}.${treeIndex + 1}` : '1'
          if (!parentRowRest) {
            rowRest.items = internalData.afterTreeFullData
          } else {
        ...
        fullData.push(row)
        let index = 0
        XEUtils.get(row, `${treeOpts.childrenField}`, []).forEach((handleRow: any) => {
          if (!handleRow._hasDeleted && (!isFilter.value || handleRow._isFilter === true)) {
            handleVirtualTreeWithBreak(handleRow, fullData, expandMaps, treeExpandedMaps, fullAllDataRowIdData, index)
            index++
          }
        })
      }
    ...
    const handleVirtualTreeToList = () => {
      ...
      if (treeConfig && treeOpts.transform) {
        ...
        internalData.afterTreeFullData.forEach((handleRow: any) => {
          handleVirtualTreeWithBreak(handleRow, fullData, expandMaps, treeExpandedMaps, fullAllDataRowIdData, 0)
        })
     ...
     */
    const loadTableData = (datas: any[], isReset: boolean, isFilter?:boolean) => {
      ...
      if (treeConfig) {
        if (transform) {
          ...
          if (isFilter) {
            treeData = [treeData.find((tree:any) => tree[treeOpts.parentField] === '')]
          }
     ...
     * @param {Array} datas 数据
     */
    const filterTableData = (isReset: boolean) => {
      ...
      // editStore.insertMaps = {}
      // editStore.removeMaps = {}
      const fullData = internalData.tableFullData
      const sYLoad = updateScrollYStatus(fullData)
      // reactData.isDragColMove = false
      // reactData.isDragRowMove = false
      // $xeTable.cacheRowMap(true)
    ...
    const insertTreeTableData = (datas: any[], isReset: boolean) => {
      ...
      tableMethods.clearMergeFooterItems()
      // 忽略排序
      tablePrivateMethods.handleTableDataWithoutSort(true)
      tableMethods.updateFooter()
      return nextTick().then(() => {
        updateHeight()
        updateStyle()
        ...
        handleReserveStatus()
        props.checkboxConfig && tablePrivateMethods.checkSelectionStatus()
        return new Promise<void>(resolve => {
          nextTick()
            .then(() => tableMethods.recalculate())
            .then(() => {
    ...
    /**
     * 从树结构中修改子节点顺序(目前只支持transform=true&lazy=false)，不会清空状态
     * @param {Array} datas 修改子节点顺序的根节点
     * @param {Array} map 修改顺序的根节点对应的子树rowid数组（仅第一层子树）
     * @author xin.zhang
     */
    const replaceTreeTableData = (datas: any[], map:any, isReset: boolean) => {
      const { fullDataRowIdData, fullAllDataRowIdData } = internalData
      const treeOpts = computeTreeOpts.value
      datas.forEach((data) => {
        const rowId = XEUtils.get(data, `${treeOpts.rowField}`)
        if (!fullDataRowIdData[rowId] || !fullAllDataRowIdData[rowId]) return
        const rowRest = fullAllDataRowIdData[rowId]
        const row = rowRest.row
        const childrenRows:any = []
        const children = map.get(rowId).reduce((rows:any[], childrenRow:any, index:number) => {
          const childrenRowid = XEUtils.get(childrenRow, `${treeOpts.rowField}`)
          const childrenRowRest = fullAllDataRowIdData[childrenRowid]
          childrenRowRest.seq = rowRest.seq + `.${index + 1}`
          childrenRowRest.treeIndex = index
          childrenRowRest.items = childrenRows
          rows.push(childrenRowRest.row)
          return rows
        }, childrenRows)
        XEUtils.set(row, `${treeOpts.childrenField}`, children)
        XEUtils.set(row, `${treeOpts.mapChildrenField}`, children.slice(0))
      })
      const fullData = internalData.tableFullTreeData.slice(0)
      // 全量数据
      internalData.tableFullData = fullData
    ...
    /**
      * 从树结构中交换两个子节点的位置（目前只支持transform=true&lazy=false）
      * @param {Array} datas 交换节点位置的根节点
      * @param {Array} map 交换节点位置的根节点对应的两个交换行index
      * @author xin.zhang
      */
    const exchangeTreeTableData = (datas: any[], map:any, isReset: boolean) => {
      const { fullDataRowIdData, fullAllDataRowIdData } = internalData
      const treeOpts = computeTreeOpts.value
      datas.forEach((data) => {
        const rowId = XEUtils.get(data, `${treeOpts.rowField}`)
        ...
        const rowRest = fullAllDataRowIdData[rowId]
        const row = rowRest.row
        const childrenRows = XEUtils.get(row, `${treeOpts.childrenField}`, [])
        const firstIndex = map.get(rowId).firstIndex
        const secondIndex = map.get(rowId).secondIndex
        const firstRowRest = fullAllDataRowIdData[XEUtils.get(childrenRows[firstIndex], `${treeOpts.rowField}`)]
        const secondRowRest = fullAllDataRowIdData[XEUtils.get(childrenRows[secondIndex], `${treeOpts.rowField}`)]
        childrenRows[firstIndex] = secondRowRest.row
        childrenRows[secondIndex] = firstRowRest.row
        firstRowRest.seq = rowRest.seq + `.${secondIndex + 1}`
        firstRowRest.treeIndex = secondIndex
        firstRowRest.items = childrenRows
        secondRowRest.seq = rowRest.seq + `.${firstIndex + 1}`
        secondRowRest.treeIndex = firstIndex
        secondRowRest.items = childrenRows
        XEUtils.set(row, `${treeOpts.childrenField}`, childrenRows)
        XEUtils.set(row, `${treeOpts.mapChildrenField}`, childrenRows.slice(0))
      })
      const fullData = internalData.tableFullTreeData.slice(0)
      // 全量数据
      internalData.tableFullData = fullData
    ...
    tableMethods = {
      ...
      /**
       * 重新渲染页面数据
       * @param {boolean} status true：表示过滤状态，页面仅展示_isFilter为true的数据；false表示清除过滤。
       * @author xin.zhang
       */
      filterData (status) {
        isFilter.value = status
        const { initStatus } = internalData
        return filterTableData(false).then(() => {
          internalData.inited = true
          internalData.initStatus = true
          if (!initStatus) {
            handleLoadDefaults()
      ...
      /**
      * 从树结构中修改子节点顺序（目前只支持transform=true&lazy=false）
      * @param {Array} datas 修改顺序的根节点数组
      * @param {Array} map 修改顺序的根节点对应的子树rowid数组（仅第一层子树）
      * @author xin.zhang
      */
      replaceTreeData (datas, map) {
        const { initStatus } = internalData
        return replaceTreeTableData(datas, map, false).then(() => {
          internalData.inited = true
          internalData.initStatus = true
          if (!initStatus) {
            handleLoadDefaults()
      ...
      /**
      * 从树结构中交换两个子节点的位置（目前只支持transform=true&lazy=false）
      * @param {Array} datas 交换节点位置的根节点数组
      * @param {Array} map 交换节点位置的根节点对应的两个交换行index
      * @author xin.zhang
      */
      exchangeTreeData (datas, map) {
        const { initStatus } = internalData
        return exchangeTreeTableData(datas, map, false).then(() => {
          internalData.inited = true
          internalData.initStatus = true
          if (!initStatus) {
            handleLoadDefaults()
    ...
    tablePrivateMethods = {
       ...
       * @author zhiliang.yang
       */
      handleTableDataWithoutSort (force?: boolean) {
        const { scrollYLoad } = reactData
        const { scrollYStore, fullDataRowIdData } = internalData
        let fullList: any[] = internalData.afterFullData
        // 是否进行数据处理
